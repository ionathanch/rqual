...
These type theoretic extensions consists of new typing rules,
...
For example, suppose we have a cons list, a function to compute its length,
and a proof that no natural is strictly less than zero.
We can define a function that safely extracts the head of a nonempty list
by using $\kw{absurd}$ to handle the impossible empty list case.
%
\begin{align*}
  \mathit{length} &: \Pi (A : \kw{Type}) \mathpunct{.} \kw{List} \; A \to \kw{Nat} \\
  \mathit{zeroNotGt} &: \forall (n : \kw{Nat}) \mathpunct{.} n < 0 \to \bot \\
  \mathit{safeHead} &: \Pi (A : \kw{Type}) (l : \kw{List} \; A) \to 0 < \mathit{length} \; l \to A \\
  \mathit{safeHead} & \; A \; \kw{nil} \; p \coloneqq \kw{absurd} \; (\kw{zeroNotGt} \; 0) \\
  \mathit{safeHead} & \; A \; (\kw{cons} \; x \; xs) \; p \coloneqq x
\end{align*}
...
Each new dependency level above the bottom would contain one additional construct
